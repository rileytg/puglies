...
impl WebSocketManager {
    ...
    /// Update RTDS connection state and emit event to frontend
    ...
    /// Update CLOB connection state and emit event to frontend
    ...
    /// Emit current connection status to frontend
    fn emit_connection_status(&self) {
        ...
        if let Err(e) = self.app.emit("connection_status", &status) {
...
mod tests {
    ...
    fn test_reconnect_delay_calculation() {
        ...
        let delay1 = WebSocketManager::calculate_reconnect_delay(1, &config);
        assert_eq!(delay1, Duration::from_secs(1));

        // Second attempt: 2 seconds
        let delay2 = WebSocketManager::calculate_reconnect_delay(2, &config);
        assert_eq!(delay2, Duration::from_secs(2));

        // Third attempt: 4 seconds
        let delay3 = WebSocketManager::calculate_reconnect_delay(3, &config);
        assert_eq!(delay3, Duration::from_secs(4));

        // Should cap at max_delay (30 seconds)
        let delay_many = WebSocketManager::calculate_reconnect_delay(10, &config);
