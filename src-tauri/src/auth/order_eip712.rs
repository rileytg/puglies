...
impl OrderSigner {
    ...
    pub fn from_private_key(private_key: &str) -> Result<Self, AppError> {
    ...
    pub async fn sign_order(&self, order: &UnsignedOrder) -> Result<SignedOrder, AppError> {
        ...
        let signature = self.signer
            ...
            .await
            .map_err(|e| AppError::Internal(format!("Failed to sign order: {}", e)))?;
    ...
    fn build_domain_separator(&self) -> Result<[u8; 32], AppError> {
    ...
    fn build_order_struct_hash(&self, order: &UnsignedOrder) -> Result<[u8; 32], AppError> {
...
fn parse_u256(s: &str) -> Result<U256, AppError> {
    ...
    if s.starts_with("0x") || s.starts_with("0X") {
        ...
        U256::from_str_radix(&s[2..], 16)
            .map_err(|e| AppError::Internal(format!("Invalid hex U256 '{}': {}", s, e)))
    } else {
        // Decimal format
        U256::from_str_radix(s, 10)
            .map_err(|e| AppError::Internal(format!("Invalid decimal U256 '{}': {}", s, e)))
    }
}
...
fn parse_address(s: &str) -> Result<Address, AppError> {
    Address::from_str(s)
        .map_err(|e| AppError::Internal(format!("Invalid address '{}': {}", s, e)))
}
